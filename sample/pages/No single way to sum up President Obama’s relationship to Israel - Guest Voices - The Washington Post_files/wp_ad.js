/*jshint evil: true, boss: true, sub: true, eqnull: true */
wpniSite = 'wpni';
...
if(!/msie 6|msie 7|msie 8/i.test(navigator.userAgent)){
  document.write('<script type="text/javascript" src="http://js.washingtonpost.com/wp-srv/ad/criteo.js"><\/script>');
}
...
//19948
if (wp_meta_data.hasOwnProperty('page_id') && wp_meta_data.page_id.toString()==="1.0.40972128"){
	commercialNode = "technology";
}

wpniAds.zoneBuilder = {
    subsection: function(){
      var a = window.wp_meta_data && wp_meta_data.subsection && wp_meta_data.subsection.toString();
      return a ? '/' + a : '';
    },
    name:function(){
...
if(/entertainment\/arkadium/.test(commercialNode) && !wpAd.arkadiumNodeHack){
...
//19626-JH
wpAd.gog_search_skin = function($){
  if(typeof $ === 'undefined'){
    document.write('<sc'+'ript type="text/javascript" src="http://ajax.googleapis.com/ajax/libs/jquery/1.7.2/jquery.min.js"></scr'+'ipt>');
  }

  document.write('<style type="text/css">body .module.gog-search, body .module.gog-search .ad-submit, body .module.gog-search .ad-ct {display:none;}</style><link rel="stylesheet" type="text/css" href="http://css.wpdigital.net/wp-adv/advertisers/target/2012/gog_skin/target.css" />');

  $(function(){
    var $submitBtn = $('<div class="ad-submit">Submit</div>').bind('click', function(e){
      $('#gog-searchForm').submit();
    });
    $('div.module.gog-search').css({'display':'block'}).append($submitBtn).append('<a href="http://www.target.com" target="_blank" class="ad-ct">Click here for target.com</a><img src="http://ad.doubleclick.net/ad/N3550.WashingtonPost/B6532577;sz=1x1;ord='+Math.floor(Math.random()*1E9)+'?" height="1" width="1" border="0" style="display:none;" alt="" />');
  });
};

wpniAds.templates.initRule('defaultTemplate');
wpniAds.templates.rules.defaultTemplate.what = ['!336x35','!336x30','!bigad','!leaderboard_2','!extra_bb','!featurebar','!tiffany_tile','!tiffany_tile_2','!336x60','!pushdown','!sponsor','!sponsor_realtor','!deal','!marketing','!nav_tile','!nn','!nn_hp','!nn_sidebar','!nn_footer','!nn_rr','!promo','!sponsor_prem_rental','!tooltile','!agoogleaday','!300x100', '!88x31','!600x130', '!336x35_top', '!marketing_2', '!pptile', '!featrent'];
...
wpniAds.templates.rules.homepage.what = ['!leaderboard','!pptile'];
wpniAds.templates.rules.homepage.where = ['washingtonpost.com'];

//10758-HS
wpniAds.templates.initRule('trulia');
wpniAds.templates.rules.trulia.what = ['bigbox'];
wpniAds.templates.rules.trulia.where = ['trulia'];

wpniAds.templates.initRule('jobs');
wpniAds.templates.rules.jobs.what = ['336x60'];
wpniAds.templates.rules.jobs.where = ['jobs'];

wpniAds.templates.initRule('local_page');
wpniAds.templates.rules.local_page.what = ['!leaderboard','!sponsor'];
...
wpniAds.templates.rules.rentals.what = ['sponsor_rental','sponsor_prem_rental', 'featrent', 'featrent_2', 'featrent_3', '!featrent_4', '!featrent_5', '!featrent_6', '!featrent_7', '!featrent_8'];
wpniAds.templates.rules.rentals.where = ['rentals'];

wpniAds.templates.initRule('liveonlineflex');
wpniAds.templates.rules.liveonlineflex.what = ['!flex_ss_bb_hp'];
wpniAds.templates.rules.liveonlineflex.where = ['multimedia/livevideo'];

//19879-CD
wpniAds.templates.initRule('arkadium_no_bb');
wpniAds.templates.rules.arkadium_no_bb = {
  what : ['!bigbox'],
...
//20187-CD
wpniAds.templates.initRule('pp_tile');
wpniAds.templates.rules.pp_tile = {
  what : ['pptile'],
...
wpniAds.templates.rules.servicealley = {
  ...
  when : [''],
  hardcodes : /http\:\/\/qaprev\./.test(location.href) ?
...
//MS - Test to render js tiffany tiles
if(/js_tiff/.test(location.search)){
  wpniAds.templates.initRule('js_tiff');
  wpniAds.templates.rules.js_tiff = {
...
if(/test_ads\=promotile/.test(location.search) && window.jQuery){
  $(function(){
...
function posMaker(adType,posOverride,delivery)
{
    ...
    {
        returnValue = adType + ((posOverride) ? '_' + posOverride : '');
    }
    else
    {
...
wpniAds.kv = {
    articleId : {
        exec : function(){
            if(wp_meta_data.contentType.toString()==="CompoundStory"){
    ...
    pageId : {
        exec : function(){
            return ('pageId='+wp_meta_data.page_id.toString().replace(/\./g,'-')+';');
        },
        check : wp_meta_data.page_id ? true : false
    },
    front : {
        exec : function(){
            return 'front='+ (wp_meta_data.contentType.toString()==='front' ? 'y' : 'n') + ';';
        },
        check : wp_meta_data.contentType ? true : false
    },
    metakw : {
        exec : function(){
            var obj = {
              '!c' : {
                ...
                financial_crisis : ['corrupt', 'goldman', 'aig', 'foreclosure', 'enron', 'sec', 'mortgage', 'Insurance', 'health', 'bank', 'wall street', 'protest', 'labor strike', 'union strike', 'labor issue', 'union issue', 'teacher strike', 'teachers strike' ],
            ...
            for(key in obj){
                for(val in obj[key]){
                    ...
                    while(len--){
                        regex += (obj[key][val][len] + '(|s|es|ed|ing|ers|ion)' + (len !== 0 ? '\\b|' : '') + '\\b');
                    }
...
function hackBin(_arg,currentLoc,what,delivery,onTheFly){
  ...
  //19646-CD- Placed here to execute before tiffany_manager is added and on first ad call:
  if(!wpAd.gogFrontFix){
    if(/cityguide/.test(commercialNode) && window.commercialPageType && /front/.test(commercialPageType)){
      commercialNode = commercialNode + '/front';
    }
    wpAd.gogFrontFix = true;
  }
    ...
    switch(_arg)
    {
        case 'onTheFly':
            ...
            //18593-personalpost
            //19648-AL
            if(/washingtonpost\.com|personalpost|obituaries|weather|jobs\/search/.test(commercialNode)){
              hackReturnValue += '!c=intrusive;';
            }
            
            //19879-CD
            if(/entertainment\/arkadium/.test(commercialNode)){
              hackReturnValue += '!c=intrusive;';
            }

            //19779-CD-prevent expandables from bottom lb:
            if(what === 'leaderboard|2' || what === 'leaderboard_2'){
              hackReturnValue += '!c=media;';
            }
        ...
        case 'what':
            ...
            //20214
            if(/cityguide/.test(currentLoc) && what === 'tiffany_tile' && /msie 7/i.test(navigator.userAgent) && document.getElementById('tiffanyAd-hItem')){
              document.getElementById('tiffanyAd-hItem').style.width = '215px';
            }
            //19626
            /*if(/cityguide/.test(currentLoc) && what === 'leaderboard' && !/cityguide\/restaurants\/spring\d{4}guide/.test(currentLoc) && (!window.mainNavClass || (window.mainNavClass && !/movies/i.test(window.mainNavClass))) && ((window.estNowWithYear && estNowWithYear >= '201205270000' && estNowWithYear <= '201206232359') || /gog_search_skin/.test(location.search))){
              wpAd.gog_search_skin(jQuery);
            }*/
            ...
            if(what==='pushdown' && currentLoc==='washingtonpost.com'){
              //20007-CD
              var wpni_adi_pushdown = document.getElementById('wpni_adi_pushdown');
              if(wpni_adi_pushdown){
                wpni_adi_pushdown.style.backgroundImage = 'url(http://img.wpdigital.net/wp-adv/advertisers/generic/pushdown/pushdown-ad.png)';
                wpni_adi_pushdown.style.backgroundPosition = '-7px -100px';
              }              
            ...
            if(what==='tiffany_tile' && !urlCheck('allAds') && !/js_tiff/.test(location.search)){
        ...
        case 'currentLoc':
            ...
            if(currentLoc.match(/classified/) && what === 'leaderboard' && typeof jQuery !== 'undefined' && jQuery.browser.msie && parseInt(jQuery.browser.version, 10) < 8){
...
if ( (typeof PIGGY_BACK_ALREADY_CALLED == "undefined" || !PIGGY_BACK_ALREADY_CALLED) && !location.href.match('https://') ) {
   if ( !window.TWP || !TWP.Constants || !TWP.Constants.AllowDocwrite || typeof TWP.Constants.AllowDocwrite.IN_ADS == 'undefined' || TWP.Constants.AllowDocwrite.IN_ADS ) {
      document.write('<script src="http://js.washingtonpost.com/wp-srv/javascript/piggy-back-on-ads.js"><\/script>');
   } else {
