...
wpAd.flags = {
  dfpUpgrade: !!/dfpupgrade/i.test(location.search)
};
...
wpAd.ppwidgetCheck = function(){
  return wpAd.cache.hasOwnProperty('ppwidget') ? wpAd.cache.ppwidget : (function(){
    wpAd.cache.ppwidget = urlCheck('tid', {type:'variable'}) === 'pp_stream' ? 'ppwidget=1;' : '';
...
function urlCheck( arg )
{
  ...
  if(arguments[1] && typeof arguments[1]=='object'){
    var obj = arguments[1];
    regex = (obj.type=='variable') ? new RegExp( "[\\?&;]"+arg+"=([^&#?]*)" ) : new RegExp( arg ) ;
  }
  else{
...
function doubleClickTestCode(testFlagArray)
{
  ...
  if(typeof this.dctCodeValue == 'undefined')
  {
    ...
    this.dctCodeValue =  ( queryResult && queryResult !== '' ) ? ((wpAd.flags.dfpUpgrade ? 'testads=' : 'kw=test_') + queryResult + ';') : '';    
  }
...
function timeScope()
{
 ...
 {
  if (parseInt(getCookie('popUpClockCookie'),10) > finalNumOfPops )
  {
    returnValue24 = false;
  }
  else
  {
   setCookie('popUpClockCookie','' + (parseInt(getCookie('popUpClockCookie'), 10) + 1) + '', cookieExpStringMaker((hourScope * 60 * 60 * 1000),thisDate()), '/', wpniDomain,'');
...
function canHaveInterstitial(wpniDomain) {
  var a = 'wp_pageview', b = getCookie(a), c = true, d = new Date(parseInt(new Date().getTime(), 10) + 432E5).toString();
  if (b && b !== '') {
...
function new_interstitial (b) {
  ...
  //19879-CD-arkadium
  g = commercialNode.match(/personalpost|entertainment\/arkadium/);
...
//10327-RZ
/*function user_id_kv(){
  if(wpAd.cache.newUKeyvalue){
    return uKeyvalue();
  } else {
    return ( getCookie('s_vi') )? 'u=o*' + charToCodeAt( getCookie('s_vi') ) + ';' : '' ;
  }
}
wpAd.cache.newUKeyvalue = !!(/test_u_keyvalue/.test(location.search));*/
...
function uKeyvalue(){
  return wpAd.cache.hasOwnProperty('u') ? wpAd.cache.u : (function(){
    ...
    var s_vi = getCookie('s_vi'),
      ...
      //s = window.TWP && TWP.Data && TWP.Data.Tracking && TWP.Data.Tracking.props && TWP.Data.Tracking.props.page_name ? TWP.Data.Tracking.props.page_name.replace(/ /g, '_') : '',
      rv = '';
    ...
    if(s_vi){
      ...
      if(s_vi){
        ...
        //get page type
        rv += ',,,' + (m.contentType && window.wpniAds && wpniAds.contentType && wpniAds.contentType[m.contentType.toString()] ? wpniAds.contentType[m.contentType.toString()] : 'article') + ',abc;';
      }
    }
...
{  
  ...
  this.templateCheck = function(currentLoc,thisPos){
    ...
    for(var a in wpniAds.templates.rules){
      ...
      for(var b = 0; b < totalFlights; b++){  
        if(locationCheck(wpniAds.templates.rules[a].where[b],currentLoc) && dateCheck(wpniAds.templates.rules[a].when[b], dateToString())){
          ...
          for(var c = 0; c < wpniAds.templates.rules[a].what.length; c++){
            ...
            testNoExclam = (testNoExclam.charAt(0) != '!')?testNoExclam:testNoExclam.substring(1,wpniAds.templates.rules[a].what[c].length);
...
{  
  ...
  currentLoc = hackBin('currentLoc',currentLoc,what,delivery,onTheFly);
  what = hackBin('what',currentLoc,what,delivery,onTheFly);
  delivery = hackBin('delivery',currentLoc,what,delivery,onTheFly);
  onTheFly= hackBin('onTheFly',currentLoc,what,delivery,onTheFly);
  
  
  var old_ajax = false;  
  if(what.match('AJAX')){
    delivery = 'AJAX';
    what = what.replace(/AJAX/,"|");
    old_ajax = true;
  }
  ...
  var adClasses = typeof adClasses !== 'undefined' ? adClasses : [];
...
function getInlineAdGraf(thisAdType) {
  ...
  if(document.getElementById(thisAdType.obstacleId)){
      ...
      bottom_of_obstacle = obstacle.offsetTop+obstacle.offsetHeight;
    }
...
googleAds = {
  ...
  execute : function (cNode,adCount,test){
    ...
    if(!document.getElementById('googleBottomBox') || googleAds.vertCheck('googleBottomBox','googleRightBox') ){
      ...
      this.debug();
      this.googleVars.google_skip += parseInt(adCount, 10);
    }
    else{
      this.debug();
