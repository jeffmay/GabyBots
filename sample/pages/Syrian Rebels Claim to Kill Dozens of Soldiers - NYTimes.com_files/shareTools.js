/*
$Id: shareTools.js 113996 2012-10-18 21:50:34Z reed.emmons $
...
NYTD.shareTools = (function($) {
    ...
    var templates = {
        ...
        <div id="shareToolsOverlayContainer" class="shareToolsOverlayContainer shareToolsInstance shareToolsThemeClassic" style="display:none;">\
            ...
            <div id="shareToolsDialogBox" class="shareToolsDialogBox">\
                <div class="shareToolsInset">\
                    <div class="shareToolsHeader shareToolsOpposingFloatControl shareToolsWrap"><a href="#" class="element2 shareToolsDialogBoxClose" id="shareToolsDialogBoxClose">Close</a><h5 class="element1">Share</h5></div>\
                    <div id="shareToolsDialogBoxContent" class="shareToolsDialogBoxContent shareToolsSingleRule">\
                        <div class="shareToolsBox shareToolsWrap"><div class="shareToolsColumn"><div class="shareToolsInset"><ul class="shareToolsList">\
                        <% NYTD.jQuery.each(shares, function(index, share) { %>\
                        <li data-share="<%= share.name %>" class="<%= share.classes %>"><span><%= share.label %></span></li>\
                        <% }); %></ul></div></div><div class="shareToolsColumn shareToolsLastColumn"><div class="shareToolsInset"><ul class="shareToolsList"></ul></div></div></div>\
                    </div>\
                    <div id="shareToolsFooter" class="shareToolsFooter"><div id="<%= options.adPosition %>" class="<%= options.adPosition %>"></div></div>\
                </div>\
            </div>\
        </div>\
        ',
        
        shareList : '<div class="shareToolsBox"><ul class="shareToolsList">\
        <% NYTD.jQuery.each(shares, function(index, share) { if (share.name == "ad") { %>\
            <li data-share="<%= share.name %>" class="<%= share.classes %> <%= share.adPosition %>" id="<%= share.adPosition %>"></li>\
        <% } else if (share.type == "link") { %>\
            <li data-share="<%= share.name %>" class="<%= share.classes %>"><a href="<%= share.postUrl %>"><%= share.label %></a></li>\
        <% } else { %>\
            <li data-share="<%= share.name %>" class="<%= share.classes %>"><span><%= share.label %></span></li>\
        <% }\
        }); %></ul></div>\
        ...
        emailThisForm : [
            '<form action="<%= data.postUrl %>" enctype="application/x-www-form-urlencoded" id="shareToolsEmailThis" name="shareToolsEmailThis" method="POST">',
                '<input type="hidden" value="1" name="type">',
                '<input type="hidden" value="<%= data.url %>" name="url">',
                '<input type="hidden" value="<%= data.title %>" name="title">',
                '<input type="hidden" value="" name="description">',
                '<input type="hidden" value="" name="asset_id">',
                '<input type="hidden" value="" name="pub_date">',
                '<input type="hidden" value="" name="author">',
                '<input type="hidden" value="" name="col_name">',
                '<input type="hidden" value="" name="source">',
                '<input type="hidden" value="" name="section">',
                '<input type="hidden" value="" name="nytdsection">',
                '<input type="hidden" value="" name="nytdsubsection">',
                '<input type="hidden" value="" name="adx_setup_tag">',
                '<input type="hidden" value="" name="adx_keywords">',
                '<input type="hidden" value="<%= data.hash %>" name="encrypted_key">',
            '</form>'
        ].join(' ')
    };
    ...
    var tools = {
        ...
        email : {
            ...
            onShowAll : true,
            label : "E-mail",
            formAction : NYTD.Hosts.wwwHost + "/mem/emailthis.html",
            postType : "popup"
        },
        permalink : {
            active : true,
    ...
    function init(el, data) { // initialize and build a new shareTool
        ...
        if (!el.hasClass('shareToolsInstance')) {
            ...
            el.html(NYTD.Template(templates.shareList, { shares : templateData })); // Inject new markup
    ...
    function initOverlay() {
        ...
        $('body').append(NYTD.Template(templates.overlay, { shares : templateData , options : { adPosition : config.defaultOverlayAdPosition }}));
    ...
    function initEmailThisForm(url, title) {
        var hash = $('meta[name=emailThisHash]').attr('content') || '';
        var templateData = {
                postUrl : tools.email.formAction,
                url : url,
                title : title,
                hash : hash
            };
            
        $('body').append(NYTD.Template(templates.emailThisForm, { data : templateData }));
        config.emailThisFormInitiated = true;
    }
    ...
    function setupHandlers() {
        ...
        $('body').delegate(selector, 'click', function(event) { // watch for clicks on selector and fire share function
            ...
            if (shareName === 'showall') { // Click Show All
                ...
                if (isLoggedIn()) {
                    if (config.emailThisFormInitiated === false) {
                        initEmailThisForm(data.url, data.title);
                    }
                    $('#shareToolsEmailThis').submit();
                } else {
                    window.location.href = 'https://myaccount.nytimes.com/auth/login?URI=' + getURI();
                }
            }
    ...
    function share(share, data) {
        ...
        var url = buildUrl(share, data);

        window.open(url, share + 'Share', 'toolbar=0,status=0,height=' + height + ',width=' + width + ',scrollbars=yes,resizable=yes');
    }
